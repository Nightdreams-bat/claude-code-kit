...
Vault-first. Use `$OBSIDIAN_VAULT_PATH` (`D:\V A U L T\Brain`).
...
pointer in `context/log.md` via `ctx log`.
...
project's `context/_map.md`, stack, and recent `log.md`. Confirm the scope in one line before
starting.
