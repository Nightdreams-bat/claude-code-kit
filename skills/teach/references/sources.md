...
```bash
python "~/.claude\skills\teach\scripts\ingest.py" "<url>" "$OBSIDIAN_VAULT_PATH\StudyVault\<Area>\_sources\<slug>.md"
```

Python lives at `<your-python-dir>\python.exe` if bare `python` resolves to the Windows Store stub.
...
```bash
python "~/.claude\skills\teach\scripts\ingest.py" "<file.pdf>" "$OBSIDIAN_VAULT_PATH\StudyVault\<Area>\_sources\<slug>.md"
```
